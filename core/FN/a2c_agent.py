...
class ActorCriticAgent(FNAgent):
    ...
    def set_updater(self, optimizer, value_loss_weight=1.0,
                    entropy_weight=0.1):
        actions = tf.compat.v1.placeholer(shape=(None), dtype="int32")
        ...
        self._updater = K.backend.function(
            ...
            outputs=[
                ...
                tf.reduce_mean(advantages), action_entropy
            ],
            updates=updates)
        ...
        def policy(self, s):
            if not self.initialized:
                ...
                action, ation_evlals, values = self.model.predict(np.array([s
                                                                            ]))
                return action[0]
...
class ActorCriticAgentTest(ActorCriticAgent):
    def make_model(self, feature_shape):
        normal = K.initalizers.glorot_normal()
        model = K.Sequential()
        model.add(
            K.layers.Dense(10,
                           input_shape=feature_shape,
                           ...
                           activation="relu"))
        model.add(
            K.layers.Dense(10, kernel_initializer=normal, activation="relu"))
        actor_layer = K.layers.Dense(len(self.actions),
                                     kernel_initializer=normal)
...
class ActorCriticTrainer(Trainer):
    def __init__(self,
                 ...
                 log_dir="",
                 file_name=""):
        super().__init_(buffer_size, batch_size, gamma, report_interval,
                        log_dir)
    ...
    def step(self, episode, step_count, agent, experience):
        ...
        if not agent.initialized:
            if len(self.experiences) < self.buffer_size:
                # Store experience untill buffer_size (enough to initialize).
                return False
