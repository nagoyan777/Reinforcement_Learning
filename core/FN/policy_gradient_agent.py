...
import gym
from fn_framework import FNAgent, Trainer, Observer, Experiences
tf.compat.v1.disable_eager_exection()
...
class PolicyGradientAgent(FNAgent):
    ...
    def set_updater(self, optimizer):
        ...
        rewards = tf.compat.v1.placeholer(shape=(None), dtype="float32")
        ...
        self._pdatter = K.backend.function(
    ...
    def episode_end(self, episode, step_count, agent):
        ...
        if not agent.initialized:
            ...
            for t, e in enumerate(self.experiences):
                s, a, r, n_s, d = e
                d_r = [
                    _r * (self.gamma**i) for i, _r in enumerate(rewards[t:])
                ]
                d_r = sum(d_r)
                d_e = Experiences(s, a, d_r, n_s, d)
                policy_experiences.append(d_e)

            agent.update(*self.make_batch(policy_experiences))

        if self.is_event(episode, self.report_interval):
            recent_rewards = self.rewrd_log[-self.report_interval:]
